...
switch (i)
{
    ...
    case 7:
    {
        ...
        if (s < 100)
        {
            Console.WriteLine("Введенное число не является трехзначным и более");
        }
        else
        {
            if (s > 99)
            {
                int a = (s / (int)Math.Pow(10,num - 2)) % 10;
                Console.WriteLine($"Третьей цифрой в числе {s} является {a}");
            }
        }
        break;
    }
